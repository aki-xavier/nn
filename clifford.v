...
pub fn (r Repr) reverse(v []f32) []f32 {
	if r.dim() >= 32 {
		// CGA reverse is purely grade-based (index = blade bitmask): + for
		// grades 0/4/5, - for grades 1/2/3; do not apply the generic flips.
		mut out := v.clone()
		for i in 0 .. 32 {
			g := bit_count(i)
			if g == 1 || g == 2 || g == 3 {
	...
	if r.dim() >= 32 {
		// CGA reverse: sign = (-1)^{g(g-1)/2} for grade g -> + for grades
		// 0/4/5, - for grades 1/2/3; index k is the blade bitmask.
		for i in 0 .. 32 {
			g := bit_count(i)
			if g == 1 || g == 2 || g == 3 {
				out[i] = -out[i]
...
// table_array_cga is the Cl(4,1) structure-constant tensor [32, 32, 32]
// generated by the generic engine.
pub fn table_array_cga() mlx.Array {
	return mlx.array_f32(gen_clifford_table(4, 1), [32, 32, 32])
}

// basis returns basis element k as a unit vector.
pub fn (r Repr) basis(k int) []f32 {
	mut v := []f32{len: r.dim()}
	v[k] = 1.0
	return v
}
...
pub fn (r Repr) prod(a []f32, b []f32) []f32 {
	match r {
		...
		.cga {
			...
			for k in 0 .. d {
				for j in 0 .. d {
					mask, sign := blade_prod(k, j, cga_sig())
					out[mask] += sign * a[k] * b[j]
				}
			}
			return out
		}
...
// cga_sig returns the Cl(4,1) squared-vector-length signature.
fn cga_sig() []f32 {
	return [f32(1), 1, 1, 1, -1]
}
...
// nz_idx returns the nonzero indices of a host vector (debug/testing aid).
pub fn nz_idx(v []f32) []int {
	mut out := []int{}
	for i, x in v {
		if x != 0 {
			out << i
